# Norme consolidée Wikidéb’IA 1.2.37
...
Un mot-clé thématique désigne normalement **un concept atomique**. La forme préférée est un nom unique ou un composé lexical stable de deux mots au plus. Une expression de trois mots ou davantage, ou contenant un connecteur tel que « de », « du », « des » ou « et », n’est admise que lorsqu’elle constitue une dénomination encyclopédique lexicalisée impossible à simplifier sans perte (par exemple `lois de la nature`, `problème du mal`, `charge de la preuve` ou `pari de Pascal`). Le vocabulaire contrôlé porte alors `multiword_exception=true` et une justification spécifique non vide.
