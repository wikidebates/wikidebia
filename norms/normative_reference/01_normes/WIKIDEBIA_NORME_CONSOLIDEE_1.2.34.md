# Norme consolidée Wikidéb’IA 1.2.34
...
> **Révision 1.2.34.** Cette révision applique la décision explicite du propriétaire autorisant une publication française autonome avant toute traduction anglaise. Un paquet peut déclarer `translation_status.en=deferred`. Dans cet état, aucun titre anglais, aucune page anglaise et aucun `{{Lien interlangue}}` ne sont requis pour une portée française. La portée anglaise est bloquée. Le passage ultérieur à `ready` ou `published` réactive les contrôles bilingues stricts et permet une reprise française destinée à ajouter les liens interlangues. Aucun titre anglais provisoire ou mécanique n'est créé. Toutes les règles 1.2.33 restent actives sauf les dispositions imposant un titre anglais et un lien interlangue avant le début réel de la traduction.
