# Norme consolidée Wikidéb’IA 1.2.33
...
> **Révision 1.2.33.** Cette révision distingue strictement la création d’une page de la modification d’une page préexistante. Les valeurs `Débat construit`, `Débat généré par IA`, `Constructed debate`, `Debate generated by AI`, `Argument généré par IA` et `Argument generated by AI` sont ajoutées uniquement lors de la création par Wikidéb’IA d’une page qui n’existait pas. Une page préexistante conserve exactement la présence ou l’absence et la valeur de ses paramètres protégés ; `débats-connexes` ou `related-debates` est préservé s’il existe et n’est pas créé s’il est absent. La révision précise également qu’une référence d’argument est retenue parce qu’elle développe ou étaye l’argument. Le fait qu’elle traite aussi d’objections n’est jamais un motif de retrait dès lors que ce premier critère est satisfait. Toutes les règles 1.2.32 restent actives.
