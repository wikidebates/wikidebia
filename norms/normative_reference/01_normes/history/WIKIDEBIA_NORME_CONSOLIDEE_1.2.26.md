# Norme consolidée Wikidéb’IA 1.2.26
...
> **Révision 1.2.26.** Cette révision corrige et complète le protocole de reprise 1.2.25 : un plan entièrement `skip` produit une attestation signée `no_changes` après relecture distante et renouvelle l’état publié sans écrire sur MediaWiki ; la sélection d’une archive exige toujours `--archive` ; les zones de staging sont nettoyées ; et les portées différées, notamment `--no-delete` puis `--only-delete`, conservent les pages encore à traiter. Toutes les règles éditoriales 1.2.25 restent actives.
