...
def _write_component_zip(path: Path, artifact: str, *, include_receipt: bool = False) -> None:
    versions = {"norm": "1.2.20", "validator": "0.4.37", "kit": "2.15.10"}
    payloads = {
    ...
    version = {"wikidebia-normes": "1.2.20", "wikidebia-validator": "0.4.37", "wikidebia-kit": "2.15.10"}[artifact]
    manifest = {
        "artifact": artifact,
        "version": version,
...
def test_component_inspector_accepts_and_verifies_optional_receipt(tmp_path: Path):
    ...
    assert metadata["versions"]["kit"] == "2.15.10"
...
def test_generated_config_is_relative_and_debate_first(tmp_path: Path):
    ...
    assert config["validator"]["required_version"] == "0.4.37"
