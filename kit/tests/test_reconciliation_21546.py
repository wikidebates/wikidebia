...
def test_reconciliation_manifest_and_publication_literals():
    ...
    assert manifest['version'] == '2.15.50'
    assert manifest['validator_version'] == '0.4.69'
    assert manifest['normative_revision'] == '1.2.66'
...
def test_reconciliation_versions_are_coherent():
    ...
    assert versions['norm'] == '1.2.66'
    assert versions['validator'] == '0.4.69'
    assert versions['kit'] == '2.15.50'
