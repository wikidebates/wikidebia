...
def write_fixture(tmp_path: Path, kind: str):
    ...
    manifest = {
      "debate_id":"demo",
      "normative_versions":{"validator":"0.4.69"},
    ...
    validator_script.write_text("import json; print(json.dumps({'validator_version':'0.4.69','result':'passed','summary':{'errors':0,'warnings':0}}))", encoding="utf-8")
    operation = {
      "id":"test",
      "kind":kind,
      "languages":["fr","en"],
    ...
    config = {
      "kit_version":"2.15.50","publication_profile":"legacy","project_root":str(tmp_path),"debate_id":"demo","corpus_root":"corpus/demo",
      "validator":{"command":[TEST_VALIDATOR_PYTHON,str(validator_script),"validate"],"required_version":"0.4.69","scopes":[],"max_warnings":0,"fingerprint_path":"validator"},
...
def test_historical_manifest_versions_do_not_block_current_validation(tmp_path):
    ...
    plan = publisher.build_plan()
    assert not plan["blockers"]
    assert plan["required_validator_version"] == "0.4.69"


def test_explicit_custom_manifest_requirement_is_still_enforced(tmp_path):
    config, path, _, _ = write_fixture(tmp_path, "full_page")
    config["manifest_requirements"] = {"normative_versions.validator": "0.4.69"}
    path.write_text(json.dumps(config), encoding="utf-8")
    corpus = tmp_path / "corpus" / "demo"
    manifest_path = corpus / "manifest.json"
    manifest = json.loads(manifest_path.read_text(encoding="utf-8"))
