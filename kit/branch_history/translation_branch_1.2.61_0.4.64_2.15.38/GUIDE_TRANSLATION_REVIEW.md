# Guide de traduction anglaise contrôlée — Kit 2.15.38
...
`name=` est un sous-titre et commence donc par une majuscule ; les keywords ordinaires suivent la capitalisation lexicale contrôlée et ne reçoivent jamais automatiquement le `name=` comme mot-clé supplémentaire. Pour une nouvelle page anglaise, les recherches de `name=` doivent provenir d'un journal réel ou d'une nouvelle vérification : ne jamais reconstruire artificiellement des requêtes historiques.
