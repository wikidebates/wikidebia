...
- score de densité source-only et unités de revue adaptatives 10/8/6/5 ;
- fermeture indépendante obligatoire des unités ;
- portée `name=` 1.2 et attestations sujet/prédicat/portée/modalité ;
- moteur sémantique 1.1 et tests de non-régression ;
- provenance explicite des recherches `name=` : `actual_log` / `fresh_recheck` / `historical_reconstruction` ;
- chaque nouvelle `Quote` porte une attestation de complétude et une seconde revue est requise en cas de ratio lexical faible ;
- la release génère un inventaire transactionnel de contenu, le lie au reçu externe et le recalcule après extraction fraîche.

## 2.15.35

- génération déterministe de `data/documentary_resources.json` dès l’application de la traduction et au scellement release ;
- normalisation DOI/URL et détection des collisions documentaires transversales avant publication ;
- activation du moteur sémantique FR→EN 1.0 dans les paquets traduits ;
- reçu de release à quatre couches de validation, avec `fresh_archive` scellé uniquement après réextraction du ZIP exact.

## 2.15.34

- alignement sur la norme 1.2.57 et le validateur 0.4.60 ;
- la revue FR→EN couvre désormais explicitement l’équivalence sémantique des titres canoniques, pas seulement des `displayed-title` ;
- la page Debate consigne un inventaire des affirmations de l’introduction, l’équivalence de `topic`/`complete-topic` et la conservation de la structure des sous-parties ;
- les attestations sont propagées dans les registres rendus afin que le validateur puisse les contrôler ;
- le verdict du validateur est présenté comme un verdict de contrôles automatisés, sans prétendre remplacer la revue sémantique humaine.

## 2.15.33

- alignement sur la norme 1.2.56 et le validateur 0.4.59 ;
