...
L’application place dans `en_content_lock.json` les paramètres de sortie déterministes : tous les paramètres documentaires sont inchangés, `citation` et `date` sont traduits, et `avertissements-citation` contient `Citation traduite par IA`, précédé de `, ` lorsqu’un avertissement existait déjà.
