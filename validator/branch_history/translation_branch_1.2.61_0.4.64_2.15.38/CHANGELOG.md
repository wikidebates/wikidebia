## 0.4.64 — 9 août 2026
- tests de régression FR→EN issus des erreurs observées ;
- signaux structurés de changement de référent, d’inférence, de suffisance et de prédicat ;
- schéma `name=` 1.2 avec portée exacte ;
- revue sémantique structurée 1.2.

## 0.4.62 — 9 août 2026

- format `argument_name_discovery_review` 1.1 avec provenance explicite des recherches et lecture rétrocompatible du format 1.0 ;
- contrôle de l’attestation de complétude `Citation`→`Quote` et seconde revue requise en cas de ratio lexical faible ;
- alignement sur l’inventaire transactionnel de release produit par le kit 2.15.36.

## 0.4.61

- registre global normalisé `documentary_resources.json` avec regroupement DOI/URL/empreinte et détection des identités incompatibles ;
- rapports à quatre couches (`structural`, `documentary`, `semantic_review`, `fresh_archive`) ;
- moteur FR→EN de marqueurs sémantiques étendu aux titres canoniques, titres affichés et résumés ;
- activation fonctionnelle par présence/version des artefacts, sans feature flag éditorial fondé sur `consolidated_norm`.

## 0.4.60 — 9 août 2026

- alignement sur la norme 1.2.57 et le kit 2.15.34 ;
- contrôle de la revue sémantique différentielle des titres canoniques anglais ;
- contrôle des attestations de fidélité de la page Debate (titre, topic, complete-topic, inventaire des affirmations et structure des sous-parties) ;
- maintien de la compatibilité 1.2.56 : les nouveaux champs 1.2.57 ne sont pas exigés rétroactivement ;
- les rapports indiquent explicitement que `passed` signifie succès des contrôles automatisés et ne remplace pas la revue humaine bilingue.
