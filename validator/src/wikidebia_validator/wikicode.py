...
PROTECTED_PAGE_PARAMETERS = {
    # Métadonnées historiques opaques. Sur une page préexistante, leur présence
    # et leur valeur ne sont jamais déduites du profil de génération.
    ("fr", "debate"): (
...
def _protected_page_parameters(ctx: PackageContext, lang: str, page_type: str) -> tuple[str, ...]:
    if _norm_at_least(ctx, "1.2.50"):
        return PROTECTED_PAGE_PARAMETERS[(lang, page_type)]
    # Contrat antérieur conservé pour les corpus verrouillés sous une norme plus
    # ancienne ; les extensions 1.2.47/1.2.49 restent reconnues.
    legacy = {
...
def _apply_page_lifecycle_contract(ctx: PackageContext, spec: dict[str, Any], tmpl: Template, lang: str, page_type: str, rel: str, page_manifest: dict[str, Any] | None) -> None:
    ...
    protected = _protected_page_parameters(ctx, lang, page_type)
    ...
    if origin == "new":
        ...
        elif name_assignment is not None and tmpl.one(name_parameter) != name_assignment.get("name"):
...
def validate_template_shape(ctx: PackageContext, tmpl: Template, lang: str, page_type: str, rel: str, page_manifest: dict[str, Any] | None = None) -> None:
    ...
    if page_type == "argument" and page_manifest is not None:
        ...
        assigned_parameter = "nom" if lang == "fr" else "name"
...
def _argument_name_assignment(ctx: PackageContext, page_id: str, lang: str) -> dict[str, Any] | None:
    ...
    if not isinstance(data, dict):
        return None
    row = next((entry for entry in data.get("entries") or []
                if entry.get("language") == lang and str(entry.get("page_id")) == page_id), None)
...
def _validate_argument_content(ctx: PackageContext, tmpl: Template, rel: str, lang: str, page_id: str, registry: dict[str, Any], page_manifest: dict[str, Any]) -> None:
    ...
    relations_omitted = (
        detailed_state.get("present") is True
        and detailed_state.get("relations_omitted") is True
        and not actual_just
        and not actual_obj
    )
    ...
    historical_absence = bool(_norm_at_least(ctx, "1.2.50") and page_manifest.get("page_origin") == "preexisting" and isinstance(preserved_date, dict) and preserved_date.get("present") is False)
    if not historical_absence and (not re.fullmatch(r"\d{4}-\d{2}-\d{2}", actual_date or "") or (expected_date and actual_date != expected_date)):
...
def _validate_interlanguage(ctx: PackageContext, tmpl: Template, rel: str, lang: str, page_type: str, page_id: str, registry: dict[str, Any], staging: bool) -> None:
    ...
    historical_interlanguage = bool(
        _norm_at_least(ctx, "1.2.50") and deferred and links and isinstance(inter_state, dict)
        and inter_state.get("present") is True and inter_state.get("value") == tmpl.one("interlangue")
    )
    ...
    if links:
        ...
        if historical_interlanguage:
            # Un lien historique attesté est du contenu à préserver, même si la
            # traduction anglaise du corpus courant est différée. Son exactitude
            # est garantie par l'état préservé, non recalculée depuis un titre EN
            # encore non verrouillé.
            if not actual_title:
...
def _historical_detailed_debate_states(ctx: PackageContext) -> dict[tuple[str, str], dict[str, Any]]:
    ...
    if cfg.get("enabled") is True and protected.intersection({"débat-détaillé", "detailed-debate"}):
        rel = cfg.get("lock_path")
...
def _validate_legacy_content_preservation(ctx: PackageContext, parsed_by_key: dict[tuple[str, str], Template]) -> None:
    ...
    if verification_revision in {"0.4.42", "0.4.43", "0.4.44", "0.4.45", "0.4.46", "0.4.47", "0.4.48", "0.4.49", "0.4.50", "0.4.51", "0.4.52", "0.4.53", "0.4.54", "0.4.55", "0.4.56"}:
    ...
    # Révision 0.4.53 : une modification de page existante n'est jamais un
    # rendu de création. Tout paramètre top-level attesté par l'inventaire
    # historique doit rester présent, sauf suppression propriétaire explicite
    # ou exception relationnelle déjà documentée pour un débat détaillé.
    if verification_revision == "0.4.53" and "all-existing-parameters" in protected_fields:
        ...
        for key, source_tmpl in source_templates.items():
            ...
            if current is None:
                # Les pages supprimées/fusionnées sont traitées par le cycle de
                # vie du registre, pas comme une disparition de paramètre.
                continue
    ...
    for key, entry in by_key.items():
        ...
        if ("résumé" if lang=="fr" else "summary") in protected_fields:
            ...
            if provenance not in {"historical_existing", "generated_after_import", "historical_absent", "owner_removed"}:
            ...
            elif verification_revision in {"0.4.42", "0.4.43", "0.4.44", "0.4.45", "0.4.46", "0.4.47", "0.4.48", "0.4.49", "0.4.50", "0.4.51", "0.4.52", "0.4.53", "0.4.54", "0.4.55", "0.4.56"} and source_tmpl is None:
            ...
            elif verification_revision in {"0.4.42", "0.4.43", "0.4.44", "0.4.45", "0.4.46", "0.4.47", "0.4.48", "0.4.49", "0.4.50", "0.4.51", "0.4.52", "0.4.53", "0.4.54", "0.4.55", "0.4.56"} and provenance == "historical_existing" and source_summary is None:
            ...
            elif verification_revision in {"0.4.42", "0.4.43", "0.4.44", "0.4.45", "0.4.46", "0.4.47", "0.4.48", "0.4.49", "0.4.50", "0.4.51", "0.4.52", "0.4.53", "0.4.54", "0.4.55", "0.4.56"} and provenance in {"generated_after_import", "historical_absent"} and source_summary is not None:
                ...
                actual = tmpl.one(field)
                if cfg.get("historical_summary_absence_revision") == "1.2.40" and actual is None:
                ...
                if verification_revision in {"0.4.42", "0.4.43", "0.4.44", "0.4.45", "0.4.46", "0.4.47", "0.4.48", "0.4.49", "0.4.50", "0.4.51", "0.4.52", "0.4.53", "0.4.54", "0.4.55", "0.4.56"} and source_summary is not None:
        ...
        if init_field in protected_fields:
            ...
            if verification_revision in {"0.4.42", "0.4.43", "0.4.44", "0.4.45", "0.4.46", "0.4.47", "0.4.48", "0.4.49", "0.4.50", "0.4.51", "0.4.52", "0.4.53", "0.4.54", "0.4.55", "0.4.56"} and source_tmpl is not None:
        ...
        if detailed_field in protected_fields:
            ...
            source_tmpl = source_templates.get(key)
            source_value = source_tmpl.one(detailed_field) if source_tmpl is not None else None
            if not isinstance(state, dict) or not isinstance(state.get("present"), bool):
                ...
                if verification_revision in {"0.4.42", "0.4.43", "0.4.44", "0.4.45", "0.4.46", "0.4.47", "0.4.48", "0.4.49", "0.4.50", "0.4.51", "0.4.52", "0.4.53", "0.4.54", "0.4.55", "0.4.56"} and source_tmpl is not None and source_value != expected:
                ...
                if verification_revision in {"0.4.42", "0.4.43", "0.4.44", "0.4.45", "0.4.46", "0.4.47", "0.4.48", "0.4.49", "0.4.50", "0.4.51", "0.4.52", "0.4.53", "0.4.54", "0.4.55", "0.4.56"} and source_value is not None:
