...
NUMERIC_CLAIM = re.compile(
    r"(?<![A-Za-zÀ-ÿ0-9])(?:\d{1,3}(?:[ \u00a0\u202f]\d{3})+|\d+)(?:[.,]\d+)?(?:\s*[-–—]\s*(?:\d{1,3}(?:[ \u00a0\u202f]\d{3})+|\d+)(?:[.,]\d+)?)?(?:\s*(?:%|pour\s+cent|percent))?(?![A-Za-zÀ-ÿ0-9])",
    re.I,
)
...
SUMMARY_SCAFFOLD_FR = re.compile(
    r"(?:plusieurs faits ou principes sont ici interprétés|cette conclusion s['’]appuie notamment sur les propositions suivantes|les faits, principes ou expériences invoqués|la thèse en tire une conséquence directe|la critique en tire une conséquence directe|le point concret est le suivant|la conséquence avancée est précise|dans le cas considéré, cela revient à dire|la proposition précise à examiner est que)",
    re.I,
)
SUMMARY_SCAFFOLD_EN = re.compile(
    r"(?:several facts or principles are interpreted here|this conclusion relies in particular on the following propositions|the facts, principles or experiences invoked|the thesis draws a direct consequence|the criticism draws a direct consequence|the concrete point is the following|the consequence advanced is precise|in the case considered, this amounts to saying)",
    re.I,
)
PRODUCTIVE_KEYWORD_FR = re.compile(
    r"^(?:limites?|histoire|construction|fiabilité|origine|sens|définition|concept|existence|preuves?|psychologie|sociologie|géographie|généalogie)\s+(?:de|du|des|de la|de l['’])\b",
    re.I,
)
PRODUCTIVE_KEYWORD_EN = re.compile(
    r"^(?:limits?|history|construction|reliability|origin|meaning|definition|concept|existence|evidence|psychology|sociology|geography|genealogy)\s+(?:of|of the)\b",
    re.I,
)
...
COMPOSITIONAL_KEYWORD_EN = re.compile(
    r"^(?:religious\s+(?:psychology|sociology|history|geography|genealogy)|science\s+and\s+religion)$",
    re.I,
)
...
DISPLAYED_TITLE_PREDICATES_FR = re.compile(
    ...
    r"indique|indiquent|montre|montrent|prouve|prouvent|soutient|soutiennent|contredit|contredisent|suggère|suggèrent|suppose|supposent|présuppose|présupposent|"
...
FR_NONVERBAL_PRECEDERS = {
    "le", "la", "les", "un", "une", "des", "du", "de", "d", "au", "aux", "à", "a", "en", "dans", "sur", "sous", "par", "pour", "avec", "sans", "entre", "vers", "chez", "ce", "cet", "cette", "ces", "son", "sa", "ses", "leur", "leurs", "notre", "nos", "votre", "vos", "mon", "ma", "mes", "ton", "ta", "tes", "quelques", "plusieurs", "certaines", "certains", "toutes", "tous", "et", "ou", "mais", "ni"
}
...
def _has_likely_finite_verb_fr(value: str) -> bool:
    ...
    """
    tokens = re.findall(r"[A-Za-zÀ-ÿ]+(?:['’][A-Za-zÀ-ÿ]+)?", value or "")
    ...
    for index, token in enumerate(lowered):
        ...
        if token in FR_COMMON_FINITE_IRREGULAR:
            return True
        # Productive finite endings. The preceding-word guard avoids treating
        # most nouns introduced by a determiner or preposition as predicates.
        if re.search(r"(?:ent|issent|issent|ait|aient|era|eront|erait|eraient|ira|iront|irait|iraient|ît|issent|e)$", token):
            if len(token) >= 5:
                return True
    return False
...
DISPLAYED_TITLE_PREDICATES_EN = re.compile(
    ...
    r"indicates?|shows?|proves?|supports?|contradicts?|suggests?|presupposes?|implies?|explains?|reveals?|confirms?|weakens?|strengthens?|"
...
def displayed_title_argument_issues(title: str, language: str) -> list[str]:
    ...
    if predicate.search(stripped):
        return []
    # The automatic gate must remain conservative: a long sentence may contain
    # a perfectly ordinary finite verb outside the deliberately small lexical
    # list. Human page-level review remains authoritative for such cases.
    words = re.findall(r"\b[\wÀ-ÿ'-]+\b", stripped)
    if language == "fr":
        auxiliary_or_impersonal = re.search(
            r"\b(?:n['’](?:a|ont|est|existe|y)|s['’][a-zà-ÿ]+|a|avons|ont|faut|y\s+a|aspire|repose|relève|évolue|évoluent|correspond|sort|furent|allait|connaît|conduit|choisit|finit|impose|mérite|souffrent|force)\b",
            stripped, re.I,
        )
...
def keyword_atomicity_issues(
    keyword: str,
    entry: dict[str, Any] | None,
    language: str | None = None,
    *,
    require_composition_attestation: bool = False,
) -> list[str]:
    ...
    lang = language or ("en" if re.search(r"\b(?:of|and|the)\b", value, re.I) else "fr")
    has_connector = bool(
        re.search(r"\b(?:de|du|des|la|le|et|of|and|the)\b", value, re.I)
        or re.search(r"\bd[’']", value, re.I)
    )
...
def lowercase_god_issues(text: str) -> list[str]:
    issues: list[str] = []
    for match in re.finditer(r"(?<![A-Za-zÀ-ÿ])dieu(?![A-Za-zÀ-ÿ])", _plain_text(text)):
        prefix = _plain_text(text)[:match.start()]
        previous_match = re.search(r"([A-Za-zÀ-ÿ’'-]+)\s*$", prefix)
...
def _active(ctx: PackageContext) -> bool:
    ...
    return (manifest.get("normative_versions") or {}).get("consolidated_norm") in {"1.1.0", "1.1.1", "1.1.2", "1.1.3", "1.1.4", "1.1.5", "1.1.6", "1.1.7", "1.1.8", "1.1.9", "1.2.0", "1.2.1", "1.2.2", "1.2.3", "1.2.4", "1.2.5", "1.2.6", "1.2.7", "1.2.8", "1.2.9", "1.2.10", "1.2.11", "1.2.12", "1.2.13", "1.2.14", "1.2.15", "1.2.16", "1.2.17", "1.2.18", "1.2.19", "1.2.20", "1.2.21", "1.2.22", "1.2.23", "1.2.24", "1.2.25", "1.2.26", "1.2.27", "1.2.28", "1.2.29", "1.2.30", "1.2.31", "1.2.32", "1.2.33", "1.2.34", "1.2.35", "1.2.36", "1.2.37", "1.2.38", "1.2.39", "1.2.40", "1.2.41", "1.2.42", "1.2.43", "1.2.44", "1.2.45", "1.2.46", "1.2.47", "1.2.48", "1.2.49", "1.2.50", "1.2.51", "1.2.52", "1.2.53"}
...
def _validate_documentary_registry(ctx: PackageContext) -> tuple[int, int]:
    ...
    pagination_errors = 0
    date_errors = 0
    norm = _consolidated_norm_from_manifest(ctx.manifest() or {})
    for source in sources.get("sources", []):
        ...
        if norm in {"1.2.9", "1.2.10", "1.2.11", "1.2.12", "1.2.13", "1.2.14", "1.2.15", "1.2.16", "1.2.17", "1.2.18", "1.2.19", "1.2.20", "1.2.21", "1.2.22", "1.2.23", "1.2.24", "1.2.25", "1.2.26", "1.2.27", "1.2.28", "1.2.29", "1.2.30", "1.2.31", "1.2.32", "1.2.33", "1.2.34", "1.2.35", "1.2.36", "1.2.37", "1.2.38", "1.2.39", "1.2.40", "1.2.41", "1.2.42", "1.2.43", "1.2.44", "1.2.45", "1.2.46", "1.2.47", "1.2.48", "1.2.49", "1.2.50", "1.2.51", "1.2.52", "1.2.53"} and isinstance(date, str) and re.fullmatch(r"\d{4}-\d{2}(?:-\d{2})?(?:[T ].*)?", date.strip()):
            date_errors += 1
        ...
        if norm in {"1.2.23", "1.2.24", "1.2.25", "1.2.26", "1.2.27", "1.2.28", "1.2.29", "1.2.30", "1.2.31", "1.2.32", "1.2.33", "1.2.34", "1.2.35", "1.2.36", "1.2.37", "1.2.38", "1.2.39", "1.2.40", "1.2.41", "1.2.42", "1.2.43", "1.2.44", "1.2.45", "1.2.46", "1.2.47", "1.2.48", "1.2.49", "1.2.50", "1.2.51", "1.2.52", "1.2.53"} and source.get("type") in {"webliography", "videography"}:
            ...
            if site and site in authors:
                ctx.report.error(
                    "WDV-DOC-004",
                    "Un auteur du registre reproduit le nom du site : une seconde recherche d’attribution est obligatoire et le champ doit être omis si aucun auteur distinct n’est identifié",
                    path=ctx.core_paths()["sources"],
                    details={"source_id": source.get("id"), "value": metadata.get("site"), "source_type": source.get("type"), "authorship_rechecked_after_site_match": verification.get("authorship_rechecked_after_site_match")},
                )
...
def _validate_debate_docs(ctx: PackageContext, manifest: dict[str, Any], controls: dict[str, Any], norm: str | None = None) -> dict[str, Any]:
    ...
    documentation_policy_1242 = controls.get("debate_documentation_policy_revision") == "1.2.42" or norm in {"1.2.42", "1.2.43", "1.2.44", "1.2.45", "1.2.46", "1.2.47", "1.2.48", "1.2.49", "1.2.50", "1.2.51", "1.2.52", "1.2.53"}
    video_authorship_policy_1242 = controls.get("video_authorship_policy_revision") == "1.2.42" or norm in {"1.2.42", "1.2.43", "1.2.44", "1.2.45", "1.2.46", "1.2.47", "1.2.48", "1.2.49", "1.2.50", "1.2.51", "1.2.52", "1.2.53"}
    if norm in {"1.2.4", "1.2.5", "1.2.6", "1.2.7", "1.2.8", "1.2.9", "1.2.10", "1.2.11", "1.2.12", "1.2.13", "1.2.14", "1.2.15", "1.2.16", "1.2.17", "1.2.18", "1.2.19", "1.2.20", "1.2.21", "1.2.22", "1.2.23", "1.2.24", "1.2.25", "1.2.26", "1.2.27", "1.2.28", "1.2.29", "1.2.30", "1.2.31", "1.2.32", "1.2.33", "1.2.34", "1.2.35", "1.2.36", "1.2.37", "1.2.38", "1.2.39", "1.2.40", "1.2.41", "1.2.42", "1.2.43", "1.2.44", "1.2.45", "1.2.46", "1.2.47", "1.2.48", "1.2.49", "1.2.50", "1.2.51", "1.2.52", "1.2.53"} and (not isinstance(profile_rationale, str) or not profile_rationale.strip()):
    ...
    for page in debate_pages:
        ...
        if (not documentation_policy_1242) and norm in {"1.2.9", "1.2.10", "1.2.11", "1.2.12", "1.2.13", "1.2.14", "1.2.15", "1.2.16", "1.2.17", "1.2.18", "1.2.19", "1.2.20", "1.2.21", "1.2.22", "1.2.23", "1.2.24", "1.2.25", "1.2.26", "1.2.27", "1.2.28", "1.2.29", "1.2.30", "1.2.31", "1.2.32", "1.2.33", "1.2.34", "1.2.35", "1.2.36", "1.2.37", "1.2.38", "1.2.39", "1.2.40", "1.2.41", "1.2.42", "1.2.43", "1.2.44", "1.2.45", "1.2.46", "1.2.47", "1.2.48", "1.2.49", "1.2.50", "1.2.51", "1.2.52", "1.2.53"}:
            insufficient = {
        ...
        if documentation_policy_1242:
            # A source belongs to one documentary orientation only. Broad surveys,
            # debates and resources developing both sides belong in the neutral bucket.
            # The validator can enforce exact identity across buckets; semantic
            # classification is attested separately in the introduction review.
            ...
            for param, items in zip(doc_params[lang], bucket_templates):
                ...
                for item in items:
                    ...
                    identity = re.sub(r"\s+", " ", link or f"{title}|{authors}").strip().casefold()
                    if not identity:
                        identity = re.sub(r"\s+", " ", re.sub(r"Référence (?:bibliographique|sitographique|vidéographique)(?: pour| contre)?", "Référence", item.raw, count=1)).strip().casefold()
                    source_orientations[identity].add(orientation)
                    source_locations[identity].append(param)
            duplicates = {
                identity: {"orientations": sorted(source_orientations[identity]), "parameters": source_locations[identity]}
                for identity in source_orientations
                if len(source_orientations[identity]) > 1
            }
            if duplicates:
                ctx.report.error(
                    "WDV-EDT-004",
                    "Une même référence documentaire est classée dans plusieurs orientations; une source couvrant plusieurs positions doit être placée dans la rubrique neutre",
                    path=page.get("file_path"),
                    details={"duplicate_count": len(duplicates), "duplicates": dict(list(duplicates.items())[:25])},
                )
        if video_authorship_policy_1242:
            ...
            for param, items in zip(doc_params[lang][6:9], bucket_templates[6:9]):
                for item in items:
                    link = (item.one("lien") or item.one("link") or "").strip()
                    authors = (item.one("auteurs") or item.one("authors") or "").strip()
                    if re.search(r"(?:youtube\.com|youtu\.be)", link, re.I) and not authors:
                        video_missing_authors.append({"parameter": param, "title": item.one("titre") or item.one("title"), "link": link})
            if video_missing_authors:
                ctx.report.error(
                    "WDV-DOC-004",
                    "Une référence vidéo YouTube de la page de débat ne précise pas le créateur ou la chaîne affichée par la plateforme",
                    path=page.get("file_path"),
                    details={"missing_count": len(video_missing_authors), "references": video_missing_authors},
                )
...
def _validate_dates(
    ...
    for page in manifest.get("pages", []):
        ...
        if not isinstance(page_expected, str) or not re.fullmatch(r"\d{4}-\d{2}-\d{2}", page_expected):
            errors += 1
        ...
        historical_date_absent = (
            policy == "per_page_preserved"
            and isinstance(preserved, dict)
            and preserved.get("present") is False
        )
        if tmpl:
...
def _validate_intro_references(ctx: PackageContext, manifest: dict[str, Any], controls: dict[str, Any]) -> dict[str, Any]:
    ...
    punctuation_policy = cfg.get("punctuation_policy_revision") == "1.2.44" or controls.get("inline_reference_punctuation_policy_revision") == "1.2.44" or norm == "1.2.44"
    ...
    for page in [p for p in manifest.get("pages", []) if p.get("page_type") == "debate"]:
        ...
        claim_driven_policy = norm in {"1.2.4", "1.2.5", "1.2.6", "1.2.7", "1.2.8", "1.2.9", "1.2.10", "1.2.11", "1.2.12", "1.2.13", "1.2.14", "1.2.15", "1.2.16", "1.2.17", "1.2.18", "1.2.19", "1.2.20", "1.2.21", "1.2.22", "1.2.23", "1.2.24", "1.2.25", "1.2.26", "1.2.27", "1.2.28", "1.2.29", "1.2.30", "1.2.31", "1.2.32", "1.2.33", "1.2.34", "1.2.35", "1.2.36", "1.2.37", "1.2.38", "1.2.39", "1.2.40", "1.2.41", "1.2.42", "1.2.43", "1.2.44", "1.2.45", "1.2.46", "1.2.47", "1.2.48", "1.2.49", "1.2.50", "1.2.51", "1.2.52", "1.2.53"}
        metrics[lang] = {
            ...
            "inline_reference_body_mode": "direct_wikicode_without_templates" if norm in {"1.2.10", "1.2.11", "1.2.12", "1.2.13", "1.2.14", "1.2.15", "1.2.16", "1.2.17", "1.2.18", "1.2.19", "1.2.20", "1.2.21", "1.2.22", "1.2.23", "1.2.24", "1.2.25", "1.2.26", "1.2.27", "1.2.28", "1.2.29", "1.2.30", "1.2.31", "1.2.32", "1.2.33", "1.2.34", "1.2.35", "1.2.36", "1.2.37", "1.2.38", "1.2.39", "1.2.40", "1.2.41", "1.2.42", "1.2.43", "1.2.44", "1.2.45", "1.2.46", "1.2.47", "1.2.48", "1.2.49", "1.2.50", "1.2.51", "1.2.52", "1.2.53"} else None,
            ...
            "terminal_period_reference_notes": terminal_period_notes,
            "punctuation_policy_revision": "1.2.44" if punctuation_policy else None,
        }
        ...
        if norm in {"1.2.10", "1.2.11", "1.2.12", "1.2.13", "1.2.14", "1.2.15", "1.2.16", "1.2.17", "1.2.18", "1.2.19", "1.2.20", "1.2.21", "1.2.22", "1.2.23", "1.2.24", "1.2.25", "1.2.26", "1.2.27", "1.2.28", "1.2.29", "1.2.30", "1.2.31", "1.2.32", "1.2.33", "1.2.34", "1.2.35", "1.2.36", "1.2.37", "1.2.38", "1.2.39", "1.2.40", "1.2.41", "1.2.42", "1.2.43", "1.2.44", "1.2.45", "1.2.46", "1.2.47", "1.2.48", "1.2.49", "1.2.50", "1.2.51", "1.2.52", "1.2.53"} and (invalid_direct_notes or missing_named):
        ...
        if norm in {"1.2.9", "1.2.10", "1.2.11", "1.2.12", "1.2.13", "1.2.14", "1.2.15", "1.2.16", "1.2.17", "1.2.18", "1.2.19", "1.2.20", "1.2.21", "1.2.22", "1.2.23", "1.2.24", "1.2.25", "1.2.26", "1.2.27", "1.2.28", "1.2.29", "1.2.30", "1.2.31", "1.2.32", "1.2.33", "1.2.34", "1.2.35", "1.2.36", "1.2.37", "1.2.38", "1.2.39", "1.2.40", "1.2.41", "1.2.42", "1.2.43", "1.2.44", "1.2.45", "1.2.46", "1.2.47", "1.2.48", "1.2.49", "1.2.50", "1.2.51", "1.2.52", "1.2.53"} and machine_dates:
...
def _wikipedia_hover_articles(content: str, lang: str) -> set[str]:
    """Backward-compatible article inventory used by the 1.2.45 review."""
...
def validate_introduction_review_data(
    ...
    norm: str | None = None,
    complete_topics: dict[str, str] | None = None,
    topics: dict[str, str] | None = None,
    introduction_policy_revision: str | None = None,
    inline_reference_punctuation_policy_revision: str | None = None,
    wikipedia_link_consistency_policy_revision: str | None = None,
    specialized_term_explanation_policy_revision: str | None = None,
    actual_contents: dict[str, dict[str, str]] | None = None,
) -> list[dict[str, Any]]:
    ...
    if norm in {"1.2.4", "1.2.5", "1.2.6", "1.2.7", "1.2.8", "1.2.9", "1.2.10", "1.2.11", "1.2.12", "1.2.13", "1.2.14", "1.2.15", "1.2.16", "1.2.17", "1.2.18", "1.2.19", "1.2.20", "1.2.21", "1.2.22", "1.2.23", "1.2.24", "1.2.25", "1.2.26", "1.2.27", "1.2.28", "1.2.29", "1.2.30", "1.2.31", "1.2.32", "1.2.33", "1.2.34", "1.2.35", "1.2.36", "1.2.37", "1.2.38", "1.2.39", "1.2.40", "1.2.41", "1.2.42", "1.2.43", "1.2.44", "1.2.45", "1.2.46", "1.2.47", "1.2.48", "1.2.49", "1.2.50", "1.2.51", "1.2.52", "1.2.53"} and review.get("normative_revision") != norm:
    ...
    for lang, titles in actual_titles.items():
        ...
        if not entry:
            ...
            continue
        required_intro_fields = INTRO_REVIEW_TRUE_FIELDS
        policy_1242 = introduction_policy_revision == "1.2.42" or norm == "1.2.42"
        policy_1243 = introduction_policy_revision == "1.2.43" or norm == "1.2.43"
        policy_1244 = inline_reference_punctuation_policy_revision == "1.2.44" or norm == "1.2.44"
        policy_1245 = wikipedia_link_consistency_policy_revision == "1.2.45" or norm == "1.2.45"
        policy_1246 = specialized_term_explanation_policy_revision in {"1.2.46", "1.2.47", "1.2.48", "1.2.49", "1.2.50", "1.2.51", "1.2.52", "1.2.53"} or norm in {"1.2.46", "1.2.47", "1.2.48", "1.2.49", "1.2.50", "1.2.51", "1.2.52", "1.2.53"}
        if policy_1242:
            required_intro_fields = tuple(field for field in INTRO_REVIEW_TRUE_FIELDS if field != "stakes_explained") + (
        ...
        if norm in {"1.2.6", "1.2.7", "1.2.8", "1.2.9", "1.2.10", "1.2.11", "1.2.12", "1.2.13", "1.2.14", "1.2.15", "1.2.16", "1.2.17", "1.2.18", "1.2.19", "1.2.20", "1.2.21", "1.2.22", "1.2.23", "1.2.24", "1.2.25", "1.2.26", "1.2.27", "1.2.28", "1.2.29", "1.2.30", "1.2.31", "1.2.32", "1.2.33", "1.2.34", "1.2.35", "1.2.36", "1.2.37", "1.2.38", "1.2.39", "1.2.40", "1.2.41", "1.2.42", "1.2.43", "1.2.44", "1.2.45", "1.2.46", "1.2.47", "1.2.48", "1.2.49", "1.2.50", "1.2.51", "1.2.52", "1.2.53"}:
        ...
        if norm in {"1.2.24", "1.2.25", "1.2.26", "1.2.27", "1.2.28", "1.2.29", "1.2.30", "1.2.31", "1.2.32", "1.2.33", "1.2.34", "1.2.35", "1.2.36", "1.2.37", "1.2.38", "1.2.39", "1.2.40", "1.2.41", "1.2.42", "1.2.43", "1.2.44", "1.2.45", "1.2.46", "1.2.47", "1.2.48", "1.2.49", "1.2.50", "1.2.51", "1.2.52", "1.2.53"}:
        ...
        if norm in {"1.2.23", "1.2.24", "1.2.25", "1.2.26", "1.2.27", "1.2.28", "1.2.29", "1.2.30", "1.2.31", "1.2.32", "1.2.33", "1.2.34", "1.2.35", "1.2.36", "1.2.37", "1.2.38", "1.2.39", "1.2.40", "1.2.41", "1.2.42", "1.2.43", "1.2.44", "1.2.45", "1.2.46", "1.2.47", "1.2.48", "1.2.49", "1.2.50", "1.2.51", "1.2.52", "1.2.53"}:
        ...
        if policy_1243:
            ...
            normalized_titles = [re.sub(r"\s+", " ", str(title or "")).strip().casefold() for title in titles]
            if expected_stakes_title.casefold() not in normalized_titles:
                ...
                stakes_content = next((value for title, value in contents.items() if re.sub(r"\s+", " ", str(title)).strip().casefold() == expected_stakes_title.casefold()), "")
                plain_stakes = re.sub(r"<ref(?:\s[^>]*)?>.*?</ref>|<ref(?:\s[^>]*)?\s*/>", " ", stakes_content, flags=re.I | re.S)
                plain_stakes = re.sub(r"\{\{.*?\}\}", " ", plain_stakes, flags=re.S)
                words = re.findall(r"[A-Za-zÀ-ÿ0-9]+", plain_stakes)
                sentences = [part for part in re.split(r"(?<=[.!?])\s+", plain_stakes.strip()) if part.strip()]
        ...
        if policy_1244:
            ...
            if not isinstance(exceptions, list):
                ...
                for exception in exceptions:
                    ...
                    if not isinstance(body_sha, str) or not re.fullmatch(r"[0-9a-f]{64}", body_sha) or body_sha in seen_exception_hashes or exception.get("complete_sentence") is not True or not isinstance(evidence, str) or len(evidence.strip()) < 12:
        ...
        if policy_1243:
            expected_stakes_title = "Enjeux du débat" if lang == "fr" else "Stakes of the debate"
            stakes_rows = [row for row in rows if isinstance(row, dict) and re.sub(r"\s+", " ", str(row.get("title") or "")).strip().casefold() == expected_stakes_title.casefold()]
...
def _validate_introduction_review(ctx: PackageContext, manifest: dict[str, Any], controls: dict[str, Any], norm: str | None) -> dict[str, Any]:
    ...
    issues = validate_introduction_review_data(
        ...
        topics=topics,
        introduction_policy_revision=controls.get("introduction_policy_revision"),
        inline_reference_punctuation_policy_revision=controls.get("inline_reference_punctuation_policy_revision"),
        wikipedia_link_consistency_policy_revision=controls.get("wikipedia_link_consistency_policy_revision"),
        specialized_term_explanation_policy_revision=controls.get("specialized_term_explanation_policy_revision"),
...
def validate_individual_review_data(
    review: Any,
    nodes: list[dict[str, Any]],
    norm: str | None = None,
    english_deferred: bool = False,
    displayed_title_policy_revision: str | None = None,
    displayed_title_policy_node_ids: set[str] | None = None,
) -> list[dict[str, Any]]:
    ...
    for node_id, node in active.items():
        ...
        if norm in {"1.2.19", "1.2.20", "1.2.21", "1.2.22", "1.2.23", "1.2.24", "1.2.25", "1.2.26", "1.2.27", "1.2.28", "1.2.29", "1.2.30", "1.2.31", "1.2.32", "1.2.33", "1.2.34", "1.2.35", "1.2.36", "1.2.37", "1.2.38", "1.2.39", "1.2.40", "1.2.41", "1.2.42", "1.2.43", "1.2.44", "1.2.45", "1.2.46", "1.2.47", "1.2.48", "1.2.49", "1.2.50", "1.2.51", "1.2.52", "1.2.53"}:
        ...
        if norm in {"1.2.22", "1.2.23", "1.2.24", "1.2.25", "1.2.26", "1.2.27", "1.2.28", "1.2.29", "1.2.30", "1.2.31", "1.2.32", "1.2.33", "1.2.34", "1.2.35", "1.2.36", "1.2.37", "1.2.38", "1.2.39", "1.2.40", "1.2.41", "1.2.42", "1.2.43", "1.2.44", "1.2.45", "1.2.46", "1.2.47", "1.2.48", "1.2.49", "1.2.50", "1.2.51", "1.2.52", "1.2.53"}:
            ...
            for lang, data in reviewed_languages:
                ...
                if displayed_title_policy_revision != "1.2.42" and norm != "1.2.42":
                ...
                elif canonical and canonical != displayed and (displayed_title_policy_node_ids is None or node_id in displayed_title_policy_node_ids):
...
def validate_graph_placement_review_data(review: Any, registry: dict[str, Any], norm: str | None = None) -> list[dict[str, Any]]:
    """Validate the occurrence-by-occurrence semantic placement ledger introduced in 1.2.20."""
    issues: list[dict[str, Any]] = []
    if not isinstance(review, dict):
        return [{"reason": "missing_or_invalid_document"}]
    if norm in {"1.2.20", "1.2.21", "1.2.22", "1.2.23", "1.2.24", "1.2.25", "1.2.26", "1.2.27", "1.2.28", "1.2.29", "1.2.30", "1.2.31", "1.2.32", "1.2.33", "1.2.34", "1.2.35", "1.2.36", "1.2.37", "1.2.38", "1.2.39", "1.2.40", "1.2.41", "1.2.42", "1.2.43", "1.2.44", "1.2.45", "1.2.46", "1.2.47", "1.2.48", "1.2.49", "1.2.50", "1.2.51", "1.2.52", "1.2.53"} and review.get("normative_revision") != norm:
    ...
    for oid, occ in occ_by_id.items():
        ...
        if depth == 1:
            ...
            if not isinstance(review_main, dict):
                ...
                continue
            required_true = (
                "direct_answer_to_debate",
                "autonomous_without_parent",
                "organizes_distinct_argument_family",
            )
            required_false = (
                "more_general_nonduplicate_parent_available",
                "principally_supports_or_attacks_specific_argument",
                "principally_example_or_specialization",
            )
...
def _validate_graph_placement_review(ctx: PackageContext, registry: dict[str, Any], controls: dict[str, Any], norm: str) -> dict[str, Any]:
    ...
    if issues:
        ctx.report.error(
            "WDV-EDT-022",
            "Revue du placement des arguments absente ou incohérente",
            path=rel or "manifest.json",
            details={"issue_count": len(issues), "issues": issues[:40]},
        )
    entries = review.get("entries", []) if isinstance(review, dict) else []
    return {
        "path": rel,
        "reviewed_occurrences": len(entries),
        "main_arguments": sum(1 for e in entries if isinstance(e, dict) and e.get("declared_depth") == 1),
        "moved_after_review": sum(1 for e in entries if isinstance(e, dict) and e.get("placement_status") == "moved_after_review"),
        "issues": len(issues),
    }
...
def _validate_individual_editorial_review(ctx: PackageContext, nodes: list[dict[str, Any]], controls: dict[str, Any]) -> dict[str, Any]:
    ...
    if controls.get("displayed_title_policy_scope") == "generated_pages_only":
        ...
        historical_ids = {
            str(row.get("id")) for row in (lock.get("arguments") or [])
            if isinstance(row, dict) and row.get("id")
        } if isinstance(lock, dict) else set()
    ...
    issues = validate_individual_review_data(
        ...
        nodes,
        norm=norm,
        english_deferred=deferred,
        displayed_title_policy_revision=controls.get("displayed_title_policy_revision"),
        displayed_title_policy_node_ids=policy_node_ids,
    )
    if issues:
...
def validate_summary_style_review_data(
    ...
    quality_1238 = quality_policy_revision == "1.2.38" or summary_policy_revision in {"1.2.39", "1.2.40", "1.2.41", "1.2.42", "1.2.43", "1.2.44", "1.2.45", "1.2.46", "1.2.47", "1.2.48", "1.2.49", "1.2.50", "1.2.51", "1.2.52", "1.2.53"} or norm in {"1.2.37", "1.2.38", "1.2.39", "1.2.40", "1.2.41", "1.2.42", "1.2.43", "1.2.44", "1.2.45", "1.2.46", "1.2.47", "1.2.48", "1.2.49", "1.2.50", "1.2.51", "1.2.52", "1.2.53"}
    if not isinstance(review, dict):
        return [{"reason": "missing_or_invalid_document"}]
    if norm in {"1.1.9", "1.2.0", "1.2.1", "1.2.2", "1.2.3", "1.2.4", "1.2.5", "1.2.6", "1.2.7", "1.2.8", "1.2.9", "1.2.10", "1.2.11", "1.2.12", "1.2.13", "1.2.14", "1.2.15", "1.2.16", "1.2.17", "1.2.18", "1.2.19", "1.2.20", "1.2.21", "1.2.22", "1.2.23", "1.2.24", "1.2.25", "1.2.26", "1.2.27", "1.2.28", "1.2.29", "1.2.30", "1.2.31", "1.2.32", "1.2.33", "1.2.34", "1.2.35", "1.2.36", "1.2.37", "1.2.38", "1.2.39", "1.2.40", "1.2.41", "1.2.42", "1.2.43", "1.2.44", "1.2.45", "1.2.46", "1.2.47", "1.2.48", "1.2.49", "1.2.50", "1.2.51", "1.2.52", "1.2.53"} and review.get("normative_revision") != norm:
        issues.append({"reason": "normative_revision", "expected": norm, "actual": review.get("normative_revision")})
    if quality_1238:
        declared_summary_policy = review.get("summary_policy_revision")
        declared_quality_policy = review.get("quality_policy_revision")
        if summary_policy_revision in {"1.2.39", "1.2.40", "1.2.41", "1.2.42", "1.2.43", "1.2.44", "1.2.45", "1.2.46", "1.2.47", "1.2.48", "1.2.49", "1.2.50", "1.2.51", "1.2.52", "1.2.53"}:
            if declared_summary_policy != summary_policy_revision:
                issues.append({"reason": "summary_policy_revision", "expected": summary_policy_revision, "actual": declared_summary_policy})
        elif declared_quality_policy != "1.2.38":
            issues.append({"reason": "quality_policy_revision", "expected": "1.2.38", "actual": declared_quality_policy})
    entries = review.get("entries")
    if not isinstance(entries, list):
        return issues + [{"reason": "missing_entries"}]
    ...
    for node_id in expected:
        ...
        for lang in expected_languages:
            ...
            if quality_1238 and key_tuple not in protected:
                ...
                summary_text = summary_map.get((node_id, lang), "")
                normalized_mechanism = re.sub(r"\s+", " ", mechanism).casefold()
                normalized_summary = re.sub(r"\s+", " ", _plain_text(summary_text)).casefold()
...
def _historically_absent_summary_keys(ctx: PackageContext) -> set[tuple[str, str]]:
    ...
    cfg = ((manifest.get("editorial_controls") or {}).get("legacy_content_preservation") or {})
    rel = cfg.get("lock_path")
    if (cfg.get("enabled") is True and cfg.get("historical_summary_absence_revision") == "1.2.40"
            and isinstance(rel, str) and ctx.exists(rel)):
        lock = ctx.load_json(rel)
        if isinstance(lock, dict):
            for entry in lock.get("arguments") or []:
...
def _validate_summary_style(
    ...
    capitalization_policy = controls.get("capitalization_policy_revision")
    summary_quality_active = quality_policy == "1.2.38" or summary_policy in {"1.2.39", "1.2.40", "1.2.41", "1.2.42", "1.2.43", "1.2.44", "1.2.45", "1.2.46", "1.2.47", "1.2.48", "1.2.49", "1.2.50", "1.2.51", "1.2.52", "1.2.53"} or norm in {"1.2.37", "1.2.38", "1.2.39", "1.2.40", "1.2.41", "1.2.42", "1.2.43", "1.2.44", "1.2.45", "1.2.46", "1.2.47", "1.2.48", "1.2.49", "1.2.50", "1.2.51", "1.2.52", "1.2.53"}
    capitalization_active = quality_policy == "1.2.38" or capitalization_policy == "1.2.39" or norm in {"1.2.37", "1.2.38", "1.2.39", "1.2.40", "1.2.41", "1.2.42", "1.2.43", "1.2.44", "1.2.45", "1.2.46", "1.2.47", "1.2.48", "1.2.49", "1.2.50", "1.2.51", "1.2.52", "1.2.53"}
    page_languages: dict[str, set[str]] = {}
    ...
    for page in manifest.get("pages", []):
        ...
        if not is_protected_historical and norm in {"1.1.9", "1.2.0", "1.2.1", "1.2.2", "1.2.3", "1.2.4", "1.2.5", "1.2.6", "1.2.7", "1.2.8", "1.2.9", "1.2.10", "1.2.11", "1.2.12", "1.2.13", "1.2.14", "1.2.15", "1.2.16", "1.2.17", "1.2.18", "1.2.19", "1.2.20", "1.2.21", "1.2.22", "1.2.23", "1.2.24", "1.2.25", "1.2.26", "1.2.27", "1.2.28", "1.2.29", "1.2.30", "1.2.31", "1.2.32", "1.2.33", "1.2.34", "1.2.35", "1.2.36", "1.2.37", "1.2.38", "1.2.39", "1.2.40", "1.2.41", "1.2.42", "1.2.43", "1.2.44", "1.2.45", "1.2.46", "1.2.47", "1.2.48", "1.2.49", "1.2.50", "1.2.51", "1.2.52", "1.2.53"} and cfg.get("opening_title_similarity_enabled", True) is True:
        ...
        claims = summary_quantitative_claims(summary) if not is_protected_historical and norm in {"1.1.9", "1.2.0", "1.2.1", "1.2.2", "1.2.3", "1.2.4", "1.2.5", "1.2.6", "1.2.7", "1.2.8", "1.2.9", "1.2.10", "1.2.11", "1.2.12", "1.2.13", "1.2.14", "1.2.15", "1.2.16", "1.2.17", "1.2.18", "1.2.19", "1.2.20", "1.2.21", "1.2.22", "1.2.23", "1.2.24", "1.2.25", "1.2.26", "1.2.27", "1.2.28", "1.2.29", "1.2.30", "1.2.31", "1.2.32", "1.2.33", "1.2.34", "1.2.35", "1.2.36", "1.2.37", "1.2.38", "1.2.39", "1.2.40", "1.2.41", "1.2.42", "1.2.43", "1.2.44", "1.2.45", "1.2.46", "1.2.47", "1.2.48", "1.2.49", "1.2.50", "1.2.51", "1.2.52", "1.2.53"} else []
        if claims:
        ...
        for (node_id, language), summary in summaries.items():
            ...
            if summary_quality_active:
                ...
                if reasons:
                    ctx.report.error(
                        "WDV-EDT-024",
                        "Résumé construit par gabarit générique, métadiscours ou énumération de pages filles",
                        path=(next((p.get("file_path") for p in manifest.get("pages", []) if p.get("page_id") == node_id and p.get("language") == language), "manifest.json")),
                        details={"node_id": node_id, "language": language, "reasons": sorted(set(reasons)), "copied_child_titles": copied_children[:6]},
                    )
            ...
            if capitalization_active and language == "fr":
                ...
                if deity_reasons:
                    ctx.report.error(
                        "WDV-EDT-026",
                        "Le nom propre « Dieu » est écrit avec une minuscule dans le résumé",
                        path=(next((p.get("file_path") for p in manifest.get("pages", []) if p.get("page_id") == node_id and p.get("language") == language), "manifest.json")),
                        details={"node_id": node_id, "reasons": deity_reasons},
                    )
        if summary_quality_active:
            for (language, sentence), node_ids in sentence_uses.items():
                if len(node_ids) >= 4:
                    ctx.report.error(
                        "WDV-EDT-024",
                        "Une même phrase de résumé est répétée dans au moins quatre pages",
                        path=review_rel or "manifest.json",
                        details={"language": language, "occurrences": len(node_ids), "node_ids": sorted(node_ids), "normalized_sentence": sentence},
                    )
    ...
    issues = validate_summary_style_review_data(
        ...
        page_languages,
        norm=norm,
        quantitative_pages=quantitative_pages,
        summaries=summaries,
        quality_policy_revision=quality_policy,
...
def validate_editorial(ctx: PackageContext) -> None:
    ...
    quality_policy = editorial_controls.get("quality_policy_revision")
    keyword_policy = editorial_controls.get("keyword_policy_revision")
    keyword_quality_active = quality_policy == "1.2.38" or keyword_policy == "1.2.39" or norm in {"1.2.37", "1.2.38", "1.2.39", "1.2.40", "1.2.41", "1.2.42", "1.2.43", "1.2.44", "1.2.45", "1.2.46", "1.2.47", "1.2.48", "1.2.49", "1.2.50", "1.2.51", "1.2.52", "1.2.53"}
    trace_controls = manifest.get("traceability_controls") or {}
    if norm in {"1.1.7", "1.1.8", "1.1.9", "1.2.0", "1.2.1", "1.2.2", "1.2.3", "1.2.4", "1.2.5", "1.2.6", "1.2.7", "1.2.8", "1.2.9", "1.2.10", "1.2.11", "1.2.12", "1.2.13", "1.2.14", "1.2.15", "1.2.16", "1.2.17", "1.2.18", "1.2.19", "1.2.20", "1.2.21", "1.2.22", "1.2.23", "1.2.24", "1.2.25", "1.2.26", "1.2.27", "1.2.28", "1.2.29", "1.2.30", "1.2.31", "1.2.32", "1.2.33", "1.2.34", "1.2.35", "1.2.36", "1.2.37", "1.2.38", "1.2.39", "1.2.40", "1.2.41", "1.2.42", "1.2.43", "1.2.44", "1.2.45", "1.2.46", "1.2.47", "1.2.48", "1.2.49", "1.2.50", "1.2.51", "1.2.52", "1.2.53"} and (not editorial_controls or not trace_controls):
        ctx.report.error("WDV-EDT-011", "Profils de contrôle déclaratifs absents du manifeste", path="manifest.json")
    if norm in {"1.1.8", "1.1.9", "1.2.0", "1.2.1", "1.2.2", "1.2.3", "1.2.4", "1.2.5", "1.2.6", "1.2.7", "1.2.8", "1.2.9", "1.2.10", "1.2.11", "1.2.12", "1.2.13", "1.2.14", "1.2.15", "1.2.16", "1.2.17", "1.2.18", "1.2.19", "1.2.20", "1.2.21", "1.2.22", "1.2.23", "1.2.24", "1.2.25", "1.2.26", "1.2.27", "1.2.28", "1.2.29", "1.2.30", "1.2.31", "1.2.32", "1.2.33", "1.2.34", "1.2.35", "1.2.36", "1.2.37", "1.2.38", "1.2.39", "1.2.40", "1.2.41", "1.2.42", "1.2.43", "1.2.44", "1.2.45", "1.2.46", "1.2.47", "1.2.48", "1.2.49", "1.2.50", "1.2.51", "1.2.52", "1.2.53"} and (not editorial_controls.get("summary_style") or not editorial_controls.get("summary_style_review_path")):
    ...
    vocab_fr, vocab_en = _load_keyword_vocabulary(ctx, editorial_controls) if norm in {"1.1.1", "1.1.2", "1.1.3", "1.1.4", "1.1.5", "1.1.6", "1.1.7", "1.1.8", "1.1.9", "1.2.0", "1.2.1", "1.2.2", "1.2.3", "1.2.4", "1.2.5", "1.2.6", "1.2.7", "1.2.8", "1.2.9", "1.2.10", "1.2.11", "1.2.12", "1.2.13", "1.2.14", "1.2.15", "1.2.16", "1.2.17", "1.2.18", "1.2.19", "1.2.20", "1.2.21", "1.2.22", "1.2.23", "1.2.24", "1.2.25", "1.2.26", "1.2.27", "1.2.28", "1.2.29", "1.2.30", "1.2.31", "1.2.32", "1.2.33", "1.2.34", "1.2.35", "1.2.36", "1.2.37", "1.2.38", "1.2.39", "1.2.40", "1.2.41", "1.2.42", "1.2.43", "1.2.44", "1.2.45", "1.2.46", "1.2.47", "1.2.48", "1.2.49", "1.2.50", "1.2.51", "1.2.52", "1.2.53"} else ({}, {})
    if norm in {"1.2.32", "1.2.33", "1.2.34", "1.2.35", "1.2.36", "1.2.37", "1.2.38", "1.2.39", "1.2.40", "1.2.41", "1.2.42", "1.2.43", "1.2.44", "1.2.45", "1.2.46", "1.2.47", "1.2.48", "1.2.49", "1.2.50", "1.2.51", "1.2.52", "1.2.53"}:
    ...
    # A corpus-specific owner decision may preserve already locked, imported displayed titles.
    # The exception is deliberately explicit, file-backed and closed: it never applies
    # to newly generated identities and does not weaken the default 10% threshold.
    displayed_title_policy_1242 = editorial_controls.get("displayed_title_policy_revision") == "1.2.42" or norm in {"1.2.42", "1.2.43", "1.2.44", "1.2.45", "1.2.46", "1.2.47", "1.2.48", "1.2.49", "1.2.50", "1.2.51", "1.2.52", "1.2.53"}
    identity_exception_ids: set[str] = set()
    identity_exception_cfg = editorial_controls.get("displayed_title_identity_exception") or {}
    if (not displayed_title_policy_1242) and identity_exception_cfg.get("enabled") is True:
        review_path = identity_exception_cfg.get("review_path")
        review = ctx.load_json(review_path) if isinstance(review_path, str) and ctx.exists(review_path) else None
        valid_exception = (
            identity_exception_cfg.get("scope") == "legacy_owner_locked_import"
            and identity_exception_cfg.get("owner_decision") is True
            and len(str(identity_exception_cfg.get("reason") or "").strip()) >= 40
            and isinstance(review, dict)
            and review.get("debate_id") == manifest.get("debate_id")
            and review.get("language") == identity_exception_cfg.get("language")
            and review.get("owner_decision") is True
            and len(str(review.get("reason") or "").strip()) >= 40
            and isinstance(review.get("node_ids"), list)
        )
        if valid_exception:
            candidate_ids = {str(x) for x in review.get("node_ids", [])}
            node_map_for_exception = {str(n.get("id")): n for n in nodes}
            invalid_ids = []
            for node_id in sorted(candidate_ids):
                node = node_map_for_exception.get(node_id)
                lang = identity_exception_cfg.get("language")
                data = (node or {}).get(lang) or {}
                if (
                    not node
                    or data.get("title_status") != "locked"
                    or (data.get("canonical_title") or "").strip().casefold() != (data.get("displayed_title") or "").strip().casefold()
                ):
                    invalid_ids.append(node_id)
            if invalid_ids:
                valid_exception = False
                ctx.report.error("WDV-EDT-001", "Dérogation de titres affichés incohérente", path=review_path, details={"invalid_node_ids": invalid_ids[:50]})
            else:
                identity_exception_ids = candidate_ids
        if not valid_exception:
            ctx.report.error("WDV-EDT-001", "Dérogation de titres affichés absente ou invalide", path=review_path or "manifest.json")
    ...
    for lang in editorial_languages:
        ...
        exact_ids = {
            ...
            if ((node.get(lang) or {}).get("canonical_title") or "").strip()
            and ((node.get(lang) or {}).get("canonical_title") or "").strip().casefold() == ((node.get(lang) or {}).get("displayed_title") or "").strip().casefold()
        }
        ...
        if (not displayed_title_policy_1242) and norm in {"1.2.22", "1.2.23", "1.2.24", "1.2.25", "1.2.26", "1.2.27", "1.2.28", "1.2.29", "1.2.30", "1.2.31", "1.2.32", "1.2.33", "1.2.34", "1.2.35", "1.2.36", "1.2.37", "1.2.38", "1.2.39", "1.2.40", "1.2.41", "1.2.42", "1.2.43", "1.2.44", "1.2.45", "1.2.46", "1.2.47", "1.2.48", "1.2.49", "1.2.50", "1.2.51", "1.2.52", "1.2.53"} and effective_ratio > 0.10:
            ctx.report.error("WDV-EDT-001", "Les titres affichés sont copiés mécaniquement depuis les titres canoniques", path=ctx.core_paths()["registry"], details={"language": lang, "ratio": ratio, "effective_ratio": effective_ratio, "owner_exception_count": len(exact_ids & exempt), "threshold": 0.10})
        cratio = dominant_classification_ratio(nodes, lang)
        classification_metrics[lang] = cratio
        if norm not in {"1.1.5", "1.1.6", "1.1.7", "1.1.8", "1.1.9", "1.2.0", "1.2.1", "1.2.2", "1.2.3", "1.2.4", "1.2.5", "1.2.6", "1.2.7", "1.2.8", "1.2.9", "1.2.10", "1.2.11", "1.2.12", "1.2.13", "1.2.14", "1.2.15", "1.2.16", "1.2.17", "1.2.18", "1.2.19", "1.2.20", "1.2.21", "1.2.22", "1.2.23", "1.2.24", "1.2.25", "1.2.26", "1.2.27", "1.2.28", "1.2.29", "1.2.30", "1.2.31", "1.2.32", "1.2.33", "1.2.34", "1.2.35", "1.2.36", "1.2.37", "1.2.38", "1.2.39", "1.2.40", "1.2.41", "1.2.42", "1.2.43", "1.2.44", "1.2.45", "1.2.46", "1.2.47", "1.2.48", "1.2.49", "1.2.50", "1.2.51", "1.2.52", "1.2.53"} and cratio > 0.90:
        ...
        threshold = 0.25 if norm in {"1.1.2", "1.1.3", "1.1.4", "1.1.5", "1.1.6", "1.1.7", "1.1.8", "1.1.9", "1.2.0", "1.2.1", "1.2.2", "1.2.3", "1.2.4", "1.2.5", "1.2.6", "1.2.7", "1.2.8", "1.2.9", "1.2.10", "1.2.11", "1.2.12", "1.2.13", "1.2.14", "1.2.15", "1.2.16", "1.2.17", "1.2.18", "1.2.19", "1.2.20", "1.2.21", "1.2.22", "1.2.23", "1.2.24", "1.2.25", "1.2.26", "1.2.27", "1.2.28", "1.2.29", "1.2.30", "1.2.31", "1.2.32", "1.2.33", "1.2.34", "1.2.35", "1.2.36", "1.2.37", "1.2.38", "1.2.39", "1.2.40", "1.2.41", "1.2.42", "1.2.43", "1.2.44", "1.2.45", "1.2.46", "1.2.47", "1.2.48", "1.2.49", "1.2.50", "1.2.51", "1.2.52", "1.2.53"} else 0.15
        if norm in {"1.1.1", "1.1.2", "1.1.3", "1.1.4", "1.1.5", "1.1.6", "1.1.7", "1.1.8", "1.1.9", "1.2.0", "1.2.1", "1.2.2", "1.2.3", "1.2.4", "1.2.5", "1.2.6", "1.2.7", "1.2.8", "1.2.9", "1.2.10", "1.2.11", "1.2.12", "1.2.13", "1.2.14", "1.2.15", "1.2.16", "1.2.17", "1.2.18", "1.2.19", "1.2.20", "1.2.21", "1.2.22", "1.2.23", "1.2.24", "1.2.25", "1.2.26", "1.2.27", "1.2.28", "1.2.29", "1.2.30", "1.2.31", "1.2.32", "1.2.33", "1.2.34", "1.2.35", "1.2.36", "1.2.37", "1.2.38", "1.2.39", "1.2.40", "1.2.41", "1.2.42", "1.2.43", "1.2.44", "1.2.45", "1.2.46", "1.2.47", "1.2.48", "1.2.49", "1.2.50", "1.2.51", "1.2.52", "1.2.53"} and dominant_keyword_set_ratio > threshold:
                ...
                if keyword_quality_active:
                    for keyword in keywords:
                        ...
                        if atomicity_reasons:
                            ...
                            ctx.report.error(
                                "WDV-EDT-025",
                                "Mot-clé non atomique ou exception multi-mots insuffisamment justifiée",
                                path=editorial_controls.get("keyword_vocabulary_path"),
                                details={"node_id": node_id, "language": lang, "keyword": keyword, "reasons": atomicity_reasons},
                            )
    ...
    if not english_deferred and norm in {"1.1.1", "1.1.2", "1.1.3", "1.1.4", "1.1.5", "1.1.6", "1.1.7", "1.1.8", "1.1.9", "1.2.0", "1.2.1", "1.2.2", "1.2.3", "1.2.4", "1.2.5", "1.2.6", "1.2.7", "1.2.8", "1.2.9", "1.2.10", "1.2.11", "1.2.12", "1.2.13", "1.2.14", "1.2.15", "1.2.16", "1.2.17", "1.2.18", "1.2.19", "1.2.20", "1.2.21", "1.2.22", "1.2.23", "1.2.24", "1.2.25", "1.2.26", "1.2.27", "1.2.28", "1.2.29", "1.2.30", "1.2.31", "1.2.32", "1.2.33", "1.2.34", "1.2.35", "1.2.36", "1.2.37", "1.2.38", "1.2.39", "1.2.40", "1.2.41", "1.2.42", "1.2.43", "1.2.44", "1.2.45", "1.2.46", "1.2.47", "1.2.48", "1.2.49", "1.2.50", "1.2.51", "1.2.52", "1.2.53"}:
        for node in nodes:
            node_id = node.get("id")
    ...
    intro_refs = _validate_intro_references(ctx, manifest, editorial_controls) if norm in {"1.1.4", "1.1.5", "1.1.6", "1.1.7", "1.1.8", "1.1.9", "1.2.0", "1.2.1", "1.2.2", "1.2.3", "1.2.4", "1.2.5", "1.2.6", "1.2.7", "1.2.8", "1.2.9", "1.2.10", "1.2.11", "1.2.12", "1.2.13", "1.2.14", "1.2.15", "1.2.16", "1.2.17", "1.2.18", "1.2.19", "1.2.20", "1.2.21", "1.2.22", "1.2.23", "1.2.24", "1.2.25", "1.2.26", "1.2.27", "1.2.28", "1.2.29", "1.2.30", "1.2.31", "1.2.32", "1.2.33", "1.2.34", "1.2.35", "1.2.36", "1.2.37", "1.2.38", "1.2.39", "1.2.40", "1.2.41", "1.2.42", "1.2.43", "1.2.44", "1.2.45", "1.2.46", "1.2.47", "1.2.48", "1.2.49", "1.2.50", "1.2.51", "1.2.52", "1.2.53"} else {}
    normative_non_regression = _validate_normative_non_regression(ctx, manifest, trace_controls) if norm in {"1.1.4", "1.1.5", "1.1.6", "1.1.7", "1.1.8", "1.1.9", "1.2.0", "1.2.1", "1.2.2", "1.2.3", "1.2.4", "1.2.5", "1.2.6", "1.2.7", "1.2.8", "1.2.9", "1.2.10", "1.2.11", "1.2.12", "1.2.13", "1.2.14", "1.2.15", "1.2.16", "1.2.17", "1.2.18", "1.2.19", "1.2.20", "1.2.21", "1.2.22", "1.2.23", "1.2.24", "1.2.25", "1.2.26", "1.2.27", "1.2.28", "1.2.29", "1.2.30", "1.2.31", "1.2.32", "1.2.33", "1.2.34", "1.2.35", "1.2.36", "1.2.37", "1.2.38", "1.2.39", "1.2.40", "1.2.41", "1.2.42", "1.2.43", "1.2.44", "1.2.45", "1.2.46", "1.2.47", "1.2.48", "1.2.49", "1.2.50", "1.2.51", "1.2.52", "1.2.53"} else {}
    individual_review = _validate_individual_editorial_review(ctx, nodes, editorial_controls) if norm in {"1.1.5", "1.1.6", "1.1.7", "1.1.8", "1.1.9", "1.2.0", "1.2.1", "1.2.2", "1.2.3", "1.2.4", "1.2.5", "1.2.6", "1.2.7", "1.2.8", "1.2.9", "1.2.10", "1.2.11", "1.2.12", "1.2.13", "1.2.14", "1.2.15", "1.2.16", "1.2.17", "1.2.18", "1.2.19", "1.2.20", "1.2.21", "1.2.22", "1.2.23", "1.2.24", "1.2.25", "1.2.26", "1.2.27", "1.2.28", "1.2.29", "1.2.30", "1.2.31", "1.2.32", "1.2.33", "1.2.34", "1.2.35", "1.2.36", "1.2.37", "1.2.38", "1.2.39", "1.2.40", "1.2.41", "1.2.42", "1.2.43", "1.2.44", "1.2.45", "1.2.46", "1.2.47", "1.2.48", "1.2.49", "1.2.50", "1.2.51", "1.2.52", "1.2.53"} else {}
    summary_style = _validate_summary_style(ctx, nodes, manifest, editorial_controls, norm) if norm in {"1.1.8", "1.1.9", "1.2.0", "1.2.1", "1.2.2", "1.2.3", "1.2.4", "1.2.5", "1.2.6", "1.2.7", "1.2.8", "1.2.9", "1.2.10", "1.2.11", "1.2.12", "1.2.13", "1.2.14", "1.2.15", "1.2.16", "1.2.17", "1.2.18", "1.2.19", "1.2.20", "1.2.21", "1.2.22", "1.2.23", "1.2.24", "1.2.25", "1.2.26", "1.2.27", "1.2.28", "1.2.29", "1.2.30", "1.2.31", "1.2.32", "1.2.33", "1.2.34", "1.2.35", "1.2.36", "1.2.37", "1.2.38", "1.2.39", "1.2.40", "1.2.41", "1.2.42", "1.2.43", "1.2.44", "1.2.45", "1.2.46", "1.2.47", "1.2.48", "1.2.49", "1.2.50", "1.2.51", "1.2.52", "1.2.53"} else {}
    graph_placement_review = _validate_graph_placement_review(ctx, registry, editorial_controls, norm) if norm in {"1.2.20", "1.2.21", "1.2.22", "1.2.23", "1.2.24", "1.2.25", "1.2.26", "1.2.27", "1.2.28", "1.2.29", "1.2.30", "1.2.31", "1.2.32", "1.2.33", "1.2.34", "1.2.35", "1.2.36", "1.2.37", "1.2.38", "1.2.39", "1.2.40", "1.2.41", "1.2.42", "1.2.43", "1.2.44", "1.2.45", "1.2.46", "1.2.47", "1.2.48", "1.2.49", "1.2.50", "1.2.51", "1.2.52", "1.2.53"} else {}
    introduction_review = _validate_introduction_review(ctx, manifest, editorial_controls, norm) if norm in {"1.2.4", "1.2.5", "1.2.6", "1.2.7", "1.2.8", "1.2.9", "1.2.10", "1.2.11", "1.2.12", "1.2.13", "1.2.14", "1.2.15", "1.2.16", "1.2.17", "1.2.18", "1.2.19", "1.2.20", "1.2.21", "1.2.22", "1.2.23", "1.2.24", "1.2.25", "1.2.26", "1.2.27", "1.2.28", "1.2.29", "1.2.30", "1.2.31", "1.2.32", "1.2.33", "1.2.34", "1.2.35", "1.2.36", "1.2.37", "1.2.38", "1.2.39", "1.2.40", "1.2.41", "1.2.42", "1.2.43", "1.2.44", "1.2.45", "1.2.46", "1.2.47", "1.2.48", "1.2.49", "1.2.50", "1.2.51", "1.2.52", "1.2.53"} else {}
    ...
    ctx.report.metrics["editorial"] = {
        ...
        "dominant_keyword_set_ratio": {lang: title_metrics.get(f"{lang}_dominant_keyword_set_ratio") for lang in editorial_languages},
        ...
        "graph_placement_review": graph_placement_review,
        "summary_style": summary_style,
        "introduction_review": introduction_review,
        "creation_date_errors": date_migration_errors,
        "traceability": trace,
        "english_translation_status": english_translation_status(manifest),
        "english_translation_deferred": english_deferred,
    }
