...
def test_compatibility_revision_lists_are_unique_and_continuous():
    ...
    expected = [f"1.2.{i}" for i in range(53, 65)]
...
def test_previous_active_norm_is_archived_and_current_norm_is_unique():
    ...
    assert (norm / "WIKIDEBIA_NORME_CONSOLIDEE_1.2.64.md").is_file()
    assert not (norm / "WIKIDEBIA_NORME_CONSOLIDEE_1.2.63.md").exists()
