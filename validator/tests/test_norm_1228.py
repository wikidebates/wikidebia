...
def test_active_normative_source_is_uniquely_1230():
    ...
    assert [path.name for path in active] == ["WIKIDEBIA_NORME_CONSOLIDEE_1.2.60.md"]
...
def test_active_citation_documents_use_quote_in_english():
    ...
    assert catalog["active_package_revision"] == "1.2.60"
...
def test_package_schema_uses_generic_norm_version_contract():
    schema = json.loads((ROOT / "src/wikidebia_validator/schemas/debate_package.schema.json").read_text(encoding="utf-8"))
    prop = json.loads((ROOT / "src/wikidebia_validator/schemas/common.schema.json").read_text(encoding="utf-8"))["$defs"]["normativeVersions"]["properties"]["consolidated_norm"]
