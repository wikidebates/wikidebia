...
def test_matrix_declares_active_revision_1260():
    ...
    assert "- **Révision :** 1.2.65" in text
...
def test_requirements_catalog_precedence_points_to_active_norm_1260():
    ...
    assert data["active_package_revision"] == "1.2.65"
    assert data["normative_revision"] == "1.2.65"
    assert "WIKIDEBIA_NORME_CONSOLIDEE_1.2.65.md" in data["precedence"]
...
def test_active_and_archived_consolidated_norms_are_coherent():
    ...
    assert active == ["WIKIDEBIA_NORME_CONSOLIDEE_1.2.65.md"]
