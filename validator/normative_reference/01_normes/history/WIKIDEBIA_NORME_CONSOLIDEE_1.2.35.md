# Norme consolidée Wikidéb’IA 1.2.35
...
> **Révision 1.2.35.** Cette révision corrige la compatibilité du mode de traduction anglaise différée. La déclaration explicite `translation_status.en=deferred` est un état opérationnel du workflow : elle peut être ajoutée à tout corpus historique compatible de la famille 1.2.x sans modifier sa `normative_versions.consolidated_norm` et sans lui appliquer rétroactivement les exigences éditoriales de versions ultérieures. Les corpus importés ou repris peuvent conserver leurs dates de création page par page avec `creation_date_policy=per_page_preserved`. Une dérogation de titres affichés hérités n'est admise que si elle est explicitement décidée par le propriétaire, limitée à des nœuds verrouillés, documentée dans un registre séparé et contrôlée sans affaiblir le seuil pour les autres pages. Toutes les règles 1.2.34 restent actives sauf contradiction explicite.
...
Pour un corpus importé, repris ou enrichi sur plusieurs dates, `editorial_controls.creation_date_policy=per_page_preserved` rend la date déclarée dans chaque entrée du manifeste autoritative pour cette page. Cette date doit correspondre exactement au wikicode canonique et au registre. Le validateur ne remplace jamais automatiquement les dates historiques par une date globale.
